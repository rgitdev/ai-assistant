...
/**
 * Command for creating user profile memories.
 * Encapsulates the logic for generating and storing user information memories.
 */
export class CreateUserProfileMemoryCommand implements CreateMemoryCommand {
  constructor(
  ...
  async execute(conversationId: string, messages: ChatMessage[]): Promise<MemoryRecord> {
    ...
    const previousMatchingMemory = prevRecords.find(r => r.category === MemoryCategory.USER_PROFILE);
  ...
  private async createMemoryInput(
    ...
    return {
      ...
      metadata: {
        ...
        createdBy: "CreateUserProfileMemoryCommand",
      },
    };
  }
}
